...
const getBooksInYear = async function (req, res) {
    ...
    let allBooks = await BookModel.find({ year: { $eq: books } })
    res.send(allBooks)
}

// getParticularBooks:- (this is a good one, make sincere effort to solve this) take any input and use it as a condition to fetch books that satisfy that conditio

const getParticularBooks = async function (req, res) {
    let books = req.query.name
    let allBooks = await BookModel.find({ authorName: { $eq: books } })
...
const getXINRBooks = async function (req, res) {
    let allBooks = await BookModel.find({ "prices.indianPrice": { $in: ["100 Inr", "200 Inr", "500 Inr"] } })
    res.send({ allBooks })
}
...
const getBooksData = async function (req, res) {
    let abc = await BookModel.find().count()
    res.send({ abc })
